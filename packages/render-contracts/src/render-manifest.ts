...
export const PreviewOutputSchema = z
  .object({
    ...
    bitrateKbps: z.number().int().min(64).max(320).default(128),
    maxDurationSeconds: z.number().int().min(5).max(120).default(30)
  })
  .strict();
...
export const RenderOutputSchema = z.object({
  ...
  lossyBitrateKbps: z.number().int().min(64).max(320).optional(),
  preview: PreviewOutputSchema.optional(),
...
export const RenderResultSchema = z
  ...
  .superRefine((value, context) => {
    if (value.status === "completed" && value.artifacts.length === 0) {
      context.addIssue({
        code: "custom",
        message: "Completed renders require at least one artifact."
      });
    }
